...
input_dir = '/Users/lauramachlab/Library/CloudStorage/OneDrive-Personal/Documents/_northwestern/_MSAI/spring 23/Computer vision/swim project/Swim Strokes'
...
with mp_pose.Pose(static_image_mode=False, min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
    ...
    for video_file in os.listdir(input_dir):
        if video_file.endswith('.mp4') or video_file.endswith('.mov'):
            ...
            overall_stroke_style = max(stroke_styles, key=stroke_styles.count)
            print('Determined stroke style: ', overall_stroke_style)

            video_count = cv2.VideoCapture(video_path)
            while video_count.isOpened():
                success, frame = video_count.read()
                ...
                if results.pose_landmarks is not None:
                    ...
                    if stroke_style_final==True:
                        ...
                        cv2.putText(frame, f'Stroke Style: {overall_stroke_style}', (20, 70), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
                    else:    
                        stroke_styles.append(stroke_style)
                        # Display the stroke style on the frame
                        cv2.putText(frame, f'Stroke Style: {overall_stroke_style}', (20, 70), cv2.FONT_HERSHEY_SIMPLEX, 1,
                                (0, 255, 0), 2)
